...
Messlaeufe auf dem Raspberry Pi 2B.
Liest Versuchsparameter aus param.csv, fuehrt fuer jeden Eintrag eine
Routenplanung durch, misst Laufzeit und CPU-Temperatur und schreibt
die Ergebnisse in results.csv.
 
Vor der eigentlichen Messreihe wird ein Vorlauf-Run ausgefuehrt,
um die CPU-Temperatur zu stabilisieren.
"""
...
warmup_map = "map_Missen_Wilhams.json"  # Karte fuer den Vorlauf-Run, individuell anpassen

def get_pi_temp():
    """Liest die aktuelle CPU-Temperatur des Raspberry Pi aus und gibt sie als float zurueck."""
...
def main():
    """Fuehrt Vorlauf-Run und anschliessend alle Messlaeufe aus param.csv durch."""
    print("-----------------------------------------------------")
    print("vorlauf")

    temp_before = get_pi_temp()
    print(f"CPU-Temperatur (vorher): {temp_before} °C")

    start_time = time.perf_counter()
    result = run_path_find.path_find(0, warmup_map,0)
    duration = time.perf_counter() - start_time

    temp_after = get_pi_temp()
    print(f"CPU-Temperatur (nachher): {temp_after} °C")
    ...
    with open(output_file, mode="a", newline="", encoding="utf-8") as results_csv:
        fieldnames = ["timestamp", "startposition", "map_file", "algo", "temp1", "temp2", "duration_sec", "steps", "route_file"]
